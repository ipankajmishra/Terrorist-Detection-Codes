	...
	upload(request, response, function (err) {
		...
		else
		{
			...
			if(str != ""){
				...
				sizeOf("Images/"+name + "_" + aadhar + "_" + str, function (err, dimensions) {
					console.log(dimensions.width, dimensions.height);
					x1 = dimensions.width;
					x2 = dimensions.height;
				});
				Jimp.read("Images/"+name + "_" + aadhar + "_" + str, function (err, lenna) {
				if (err) throw err;
					lenna.resize((x1*1.5)/3,(x2*1.5)/3)            // resize
							 .quality(100)                 // set JPEG quality
							 .write("Images/"+name + "_" + aadhar + "_" + str); // save
							 return response.redirect('/thanks.html');
			});
			}
			else{
...
app.post('/server/upload', function(request, response){
	...
	fs.readdir( './Images', (error, files) => { 
		...
		if(totalFiles==0)
		{
			return response.redirect('/importempty.html');
		}
	});
	//imp.dataloop();
